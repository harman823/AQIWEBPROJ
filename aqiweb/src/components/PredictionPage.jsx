...
export default function PredictionPage() {
  ...
  const handlePredict = async () => {
    ...
    setPrediction(null);
    try {
      const res = await axios.post('/api/predict', { city, days });
  ...
  return (
    <div className="card bg-slate-800 rounded-xl p-6 shadow-lg">
      <h2 className="text-xl font-bold mb-4 flex items-center">
        <i data-feather="cloud" className="mr-2 text-primary-400"></i>
        AQI Forecast
      </h2>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              value={`${days} days`}
              readOnly
              className="w-full bg-slate-700 text-white rounded-lg px-4 py-2 focus:outline-none"
            />
          </div>
        </div>
